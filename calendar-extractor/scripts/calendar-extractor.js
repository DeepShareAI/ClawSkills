 ...
 * Invoked by the javis-server session-dispatcher on user approval, OR by the LLM
 * on user-typed commands ("today's meetings" / "calendar extract").
 *
 * Two subcommands:
 ...
 *          single unit (the approved dispatcher run); with no flags it returns the
 ...
 *          (best-effort), and push a markdown digest of the NEW events to the
 *          user's iOS chat. The skill does NOT self-gate per unit — the server
 *          owns run-once (DispatchRouteExecuted) and human approval.
 *
 * Usage:
...
function parseArgv() {
  if (process.argv.includes('--help')) {
    console.log([
      ...
      '         --session/--kbd-input filter to one unit (the approved dispatcher run)',
...
async function doFetch(opts = {}, deps = {}) {
  ...
  // --session (audio) / --kbd-input (keyboard) target one unit for the approved
  // dispatcher run; a unit outside the default window widens --hours as needed.
...
// (DispatchRouteExecuted) and human approval. push only dedups individual
// events against the `seen` map so the same event is never delivered twice
// across overlapping manual 24h windows or a re-run.
